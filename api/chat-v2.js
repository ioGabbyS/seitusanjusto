export default async function handler(req, res) {
    ...
    try {
        ...
        const text = data.candidates?.[0]?.content?.parts?.[0]?.text || "¡Hola! Soy Tucito 🐲. ¡Vamos a festejar con un helado! 🍦";
        return res.status(200).json({ text });

    } catch (e) {
        return res.status(200).json({ text: "¡Hola! Soy Tucito 🐲. ¿En qué pueo ayudarte? 🍦" });
    }
}
