...
import { Search, Trash2, Plus, Minus, CreditCard, Banknote, Save, ArrowRight, Lock, User, Star, X, Gift, Edit, History } from 'lucide-react';
...
import { MPQRModal } from './MercadoPagoPanel';
...
export default function POS() {
    ...
    const [mpPaymentDetected, setMPPaymentDetected] = useState(false);
    ...
    const handleCheckout = async () => {
        ...
        try {
            ...
            const saleData = {
                ...
                customerDni: selectedCustomer ? selectedCustomer.dni : '',

                customerId: selectedCustomer ? selectedCustomer.id : null,
                employeeId: (paymentMethod === 'personal') ? selectedEmployeeId : null
            };
            ...
            if (savedSale && savedSale.pointsEarned > 0) {
                alert(`🎊 ¡PUNTOS SUMADOS!\n\nEl cliente sumó ${savedSale.pointsEarned} puntos con esta compra.`);
            }

            // Print Ticket
            try {
                // Calculate new total points for the ticket (Old + Earned - Redeemed)
                const currentPoints = selectedCustomer ? selectedCustomer.points : 0;
                const earned = savedSale ? savedSale.pointsEarned : 0;
                const redeemed = pointsToRedeem || 0;
                const newTotalPoints = currentPoints + earned - redeemed;

                const printData = {
                    ...saleData,
                    orderNumber: savedSale ? savedSale.orderNumber : '---',
                    pointsEarned: earned,
                    customerPoints: selectedCustomer ? newTotalPoints : undefined
                };
                await printTicket(printData);
                console.log("Ticket sent to printer", { newTotalPoints });
            } catch (printError) {
                console.error("Printing error:", printError);
                // Non-blocking
            }

            // Fiscal Invoice (Optional)
            if (shouldInvoiceFiscal) {
                try {
                    await afipService.generateInvoice(saleData);
                    alert("Factura Fiscal emitida correctamente (Simulación).");
                } catch (fiscalError) {
                    console.error("Fiscal error:", fiscalError);
                    alert("Error al emitir factura fiscal: " + fiscalError.message);
                }
            } else {
        ...
        <div className="flex flex-col md:flex-row min-h-[calc(100vh-100px)] gap-4 transition-colors duration-500">
            ...
            <div className="w-full md:w-96 bg-white dark:bg-slate-900 rounded-2xl shadow-xl border border-slate-200 dark:border-slate-800 flex flex-col flex-shrink-0">
                ...
                <div className="p-4 bg-slate-50 dark:bg-slate-900/80 border-t border-slate-200 dark:border-slate-800 space-y-3">
                    ...
                    <div className="flex gap-1 p-1 bg-white dark:bg-slate-800 rounded-lg border border-slate-200 dark:border-slate-700">
                        ...
                        <button
                            ...
                            className={`flex-1 py-1 text-[10px] md:text-xs font-bold rounded-md transition-all ${paymentMethod === 'mercadopago' ? 'text-white shadow-sm' : 'text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-700/50'}`}
                            style={paymentMethod === 'mercadopago' ? { background: '#009ee3' } : {}}
                        >
                            MP
                        </button>
                    </div>
                    ...
                    {paymentMethod === 'mercadopago' && (
                        <div className="rounded-xl border-2 border-[#009ee3]/30 bg-[#009ee3]/5 p-3 space-y-2">
                            <div className="flex items-center gap-2">
                                <div className="w-7 h-7 rounded-full flex items-center justify-center" style={{ background: '#009ee3' }}>
                                    <span className="text-white text-[10px] font-black">MP</span>
                                </div>
                                <div>
                                    <div className="text-[10px] font-black text-[#009ee3] uppercase">MercadoPago - Código QR</div>
                                    <div className="text-[9px] text-slate-500 dark:text-slate-400">Se generará el QR al confirmar el pedido</div>
                                </div>
                            </div>
                            {mpPaymentDetected && (
                                <div className="flex items-center gap-2 bg-green-50 dark:bg-green-900/20 rounded-lg p-2">
                                    <span className="text-green-500 text-xs">✅</span>
                                    <span className="text-xs font-bold text-green-700 dark:text-green-400">¡Pago recibido vía MercadoPago!</span>
                                </div>
                            )}
                        </div>
                    )}
                ...
                <button
                    ...
                    disabled={cart.length === 0 || !activeSession || isInvoicing}
                    className="w-full py-4 rounded-xl font-bold text-lg shadow-lg flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed transition-all text-white"
                    style={paymentMethod === 'mercadopago'
                        ? { background: cart.length > 0 && activeSession ? '#009ee3' : '#9ca3af', boxShadow: '0 8px 20px -4px rgba(0,158,227,0.4)' }
                        : { background: '#f97316', boxShadow: '0 8px 20px -4px rgba(249,115,22,0.3)' }
                    }
                >
                    {isInvoicing ? (
                        <>
                ...
                {/* Modal QR de MercadoPago */}
                {showMPModal && (
                    <MPQRModal
                        amount={total}
                        items={cart.map(i => ({ name: i.name, category: i.category, quantity: i.quantity, price: i.price || 0 }))}
                        cashier={activeSession?.cashier || 'Sistema'}
                        onClose={() => setShowMPModal(false)}
                        onPaymentDetected={(payment) => {
                            setMPPaymentDetected(true);
