...
const Martina15 = () => {
    const { addEventPostMartina, uploadImage, loadData } = useStore();
        ...
        return (
            <div className="min-h-screen bg-rose-50 flex items-center justify-center p-6 text-center">
                <div className="max-w-md w-full bg-white/70 backdrop-blur-2xl p-12 rounded-[3.5rem] shadow-2xl border border-white">
                    ...
                    <h1 className="text-4xl font-black text-rose-950 mb-4 tracking-tighter">¡GRACIAS!</h1>
                    <p className="text-rose-800 font-medium text-lg leading-relaxed mb-10">
                        Tu mensaje y foto se enviaron correctamente. En unos minutos aparecerán en la pantalla gigante.
                    </p>
                    <button
                        onClick={() => setIsSent(false)}
                        className="w-full py-5 bg-rose-500 hover:bg-rose-600 text-white rounded-3xl font-black text-lg transition-all shadow-xl shadow-rose-200 active:scale-95"
                    >
                        ENVIAR OTRO
                    </button>
                    <div className="mt-8 flex flex-col items-center gap-4">
                        <button
                            onClick={() => window.location.hash = '#/martina-show'}
                            className="w-full py-4 bg-white border-2 border-rose-500 text-rose-500 rounded-3xl font-black text-sm transition-all hover:bg-rose-50 uppercase tracking-widest flex items-center justify-center gap-2"
                        >
                            <Sparkles size={18} /> Ver Show en Vivo
                        </button>
                        <div className="flex items-center gap-2 text-rose-300">
                            <Heart size={16} fill="currentColor" />
                            <span className="text-[10px] font-black tracking-widest uppercase">Martina 15</span>
                            <Heart size={16} fill="currentColor" />
                        </div>
                    </div>
                </div>
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-rose-400 via-rose-300 to-fuchsia-300 p-4 sm:p-8 flex flex-col items-center">
            <div className="w-full max-w-xl text-center mb-8">
                ...
                <h1 className="text-6xl sm:text-7xl font-black text-white tracking-tighter leading-[0.8] mb-4 drop-shadow-2xl">
                    MARTINA<br /><span className="text-rose-100 italic">15</span>
                </h1>
            ...
            <div className="w-full max-w-xl bg-white/40 backdrop-blur-3xl rounded-[3rem] p-8 sm:p-12 shadow-[0_32px_64px_-16px_rgba(0,0,0,0.1)] border border-white/50">
                <form onSubmit={handleSubmit} className="space-y-8">
                    <div className="space-y-3">
                        <label className="block text-rose-950 font-black text-xs uppercase tracking-widest ml-1">Tu Nombre</label>
                        <input
                            type="text"
                            value={author}
                            onChange={(e) => setAuthor(e.target.value)}
                            placeholder="Ej: La tía María"
                            className="w-full bg-white/60 border-2 border-transparent focus:border-rose-400 focus:bg-white p-5 rounded-3xl outline-none transition-all font-bold text-rose-950 shadow-inner"
                        />
                    </div>

                    <div className="space-y-3">
                        <label className="block text-rose-950 font-black text-xs uppercase tracking-widest ml-1">Mensaje para Martina</label>
                        <textarea
                            value={comment}
                            onChange={(e) => setComment(e.target.value)}
                            placeholder="¡Felices 15 Martina! Te queremos..."
                            rows={4}
                            className="w-full bg-white/60 border-2 border-transparent focus:border-rose-400 focus:bg-white p-6 rounded-3xl outline-none transition-all font-bold text-rose-950 shadow-inner resize-none"
                        />
                    </div>
                    ...
                    <div className="space-y-4">
                        ...
                        {imagePreview ? (
                            <div className="relative rounded-3xl overflow-hidden group shadow-2xl border-4 border-white/80 aspect-video">
                                ...
                                <div className="absolute inset-0 bg-rose-900/40 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none">
                    ...
                    <button
                        ...
                        className={`w-full py-6 rounded-3xl font-black text-xl flex items-center justify-center gap-3 transition-all shadow-2xl ${isUploading || (!comment && !image)
                            ...
                            : 'bg-gradient-to-r from-rose-500 to-fuchsia-500 text-white hover:scale-105 active:scale-95 shadow-rose-300'
                            }`}
                    >
                        {isUploading ? (
                            <>
                    ...
                    <p className="text-center text-rose-800/60 font-black text-[10px] uppercase tracking-widest">
